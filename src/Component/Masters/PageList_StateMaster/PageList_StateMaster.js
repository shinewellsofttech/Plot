...
const API_URL = API_WEB_URLS.MASTER + "/0/token/StateMaster/Id/0";
...
const PageList_StateMasterContainer = () => {
  ...
  const loadData = () => {
    Fn_FillListData(dispatch, setState, "StateMasterList", API_URL);
  };
  ...
  const handleDelete = (id) => {
    ...
    if (window.confirm("Are you sure you want to delete this state?")) {
      const deleteUrl = API_WEB_URLS.MASTER + "/0/token/StateMaster/Id";
  ...
  const filteredData = (Array.isArray(state.StateMasterList) ? state.StateMasterList : []).filter((item) => {
