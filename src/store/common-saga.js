...
function* sagaDelete_Data(action) {
  ...
  console.log("sagaDelete_Data called with:", { id, apiURL, fullUrl: API_WEB_URLS.BASE + apiURL + '/' + id });
  try {
    // Some APIs don't support DELETE method, so use POST instead
    const deletePayload = { id: id };
    const response = yield call(API_HELPER.apiPOST, API_WEB_URLS.BASE + apiURL + '/' + id, deletePayload);
